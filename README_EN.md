...
- **swup single-page transitions** (SPA transitions + preload + progress bar + slide animations)
- **Built-in search** (build-time index, dialog-style search, keyword highlight)
- **Image viewer** (click to zoom, drag, wheel zoom, prev/next, EXIF panel)
- **Comment systems** (waline / giscus / gitalk / twikoo / utterances / artalk, config-driven)
- **Tag plugins**: `{% button %}` `{% callout %}` `{% note %}` `{% folding %}` `{% grid %}` `{% tabs %}`
